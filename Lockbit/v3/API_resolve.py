...
def resolve_all_APIs(resolve_ea):
    ...
    for ref in idautils.CodeRefsTo(resolve_ea, 1):
        ...
        while True:
            ...
            if idc.print_insn_mnem(prev_instruction_ea) == 'push':
                if API_addresses_ea == 0:
                    API_addresses_ea = idc.get_operand_value(
                        prev_instruction_ea, 0)
                else:
                    API_hashes_ea = idc.get_operand_value(
                        prev_instruction_ea, 0)
                    break
            curr_ea = prev_instruction_ea

        # print('API_addresses_ea:', hex(API_addresses_ea))
        ...
        index = 0
        while True:
            API_hash = int.from_bytes(idc.get_bytes(
                API_hashes_ea + 4 * index, 4), 'little')
            if API_hash == 0xCCCCCCCC:
                break
            # print("API_hash: ", hex(API_hash))
